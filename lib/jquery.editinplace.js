...
Patches welcomed! As long as they come with testcase included. For guidance see the tests at </spec/unit/spec.js> Just send them to me --Martin
...
- Expand the function commit interface so it is more controllable (does not call alert in error cases)
...
$.extend(InlineEditor.prototype, {
	...
	mouseEnter: function() {
		if (this.settings.hover_class)
			...
			this.dom.css("background", this.settings.bg_over);
	},
	...
	createEditorElement: function() {
		...
		if ("select" === this.settings.field_type)
			...
			var editor = $('<input type="text"' + this.inputNameAndClass() + '/>');
		else if ("textarea" === this.settings.field_type)
			var editor = $('<textarea' + this.inputNameAndClass() 
	...
	connectEventsToEditor: function() {
		...
		function cancelEditorAction() {
			return that.handleCancelEditor();
		}
		function saveEditorAction() {
			return that.handleSaveEditor();
		}
	...
	handleSaveEditor: function() {
		...
		if (this.settings.value_required && (enteredText == "" || enteredText == undefined)) {
			this.init();
			this.dom.html(this.originalHTML);
			// REFACT: This should be overridable in the this.settings object
		...
		// Why is this needed here? The hover should still take care of it?
		this.dom.css("background", this.settings.bg_out);
	...
	handleSubmitToCallback: function(enteredText) {
		var newHTML = this.settings.callback(this.dom.attr("id"), enteredText, this.originalHTML, this.settings.params);
		this.init();
		if (newHTML) // BUG: empty string and 0 cannot be entered this way
			this.dom.html(newHTML);
		else {
			/* failure; put original back */
			if (this.settings.callbackShowErrors) {
				// REFACT: This should be overridable in the this.settings object
				alert("Failed to save value: " + enteredText);
			}
			this.dom.html(this.originalHTML);
		}
	...
	handleSubmitToServer: function(enteredText) {
		// REFACT: encoding the original html only works if the node contains only text
		//         otherwise this will completely mangle the submit format-string
		var data = this.settings.update_value + '=' + enteredText 
			+ '&' + this.settings.element_id + '=' + this.dom.attr("id") 
			+ (('' !== this.settings.params) ? '&' + this.settings.params : '')
			+ '&' + this.settings.original_html + '=' + this.originalHTML;
