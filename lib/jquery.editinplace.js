...
- Support setting stuff via metadata plugin
	...
	return this.each(function(){
		...
		.click(function(){
			...
			if(!editing)
			{
				...
				{
					var use_field_type = '<textarea name="inplace_value" class="inplace_field" rows="' + settings.textarea_rows + '" cols="' + settings.textarea_cols + '">' + trimAndEscape(jQuery(this).text()) + '</textarea>';
				}
				else if(settings.field_type == "text")
				{
					var use_field_type = '<input type="text" name="inplace_value" class="inplace_field" value="' +
											trimAndEscape(jQuery(this).text()) + '" />';
				}
...
function trimAndEscape(aString) {
	return escapeHTML(trimWhitespace(aString));
}
