...
Patches with tests welcomed! For guidance see the tests at </spec/unit/spec.js>. To submit, just mail them to me. --Martin
...
Using the editor is extremely easy. You select some elements with jQuery and then bind editInPlace to it like so:

> $('.name').editInPlace({callback: consumerFunction})

The code above looks for an element with the class .name, makes it editable and will submit the edited value to the consumerFunction. Alternatively you could hand in a url to post the changed value to. The editor accepts a wealth of other options to customize how it behaves, so have a look at the standard options which are exposed as $.fn.editInPlace.defaults where you can look them up or even change them. For some examples of how to use the editor, have a look at </demo/index.html> and especially </demo/js/demo.js>.
...
To run the teststuite just open </spec/dom.html> in your browser of choice. (I found Webkit Nightlies to be most pleasant to debug in). For more comfort install jspec via {gem install jspec}, then you can run {jspec bind --browser <yourprobwser>} in the root directory of the project to re-execute the testsuite whenever you save changes to a file.

If you see an enhancement or happen to need one of the things mentioned in the TODO file, please don't hesitate to implement them with a testsuite to boost and send me a patch.
...
$.fn.editInPlace.defaults = {
	...
	preinit:			null, // function: this function gets called after a click on an editable element but before the editor's initial drawing. If you return false, the inline editor will not open. Prototype: function(currentDomNode)
	postclose:			null // function: this function gets called after the inline editor has closed and all values are updated. Prototype: function(currentDomNode)
};
...
$.extend(InlineEditor.prototype, {
	...
	reinit: function() {
		...
		if (this.settings.postclose)
			this.settings.postclose(this.dom);
	...
	shouldOpenEditor: function(eventTarget) {
		...
		if (this.settings.preinit) {
			var shouldContinueAccordingToPreinit = false !== this.settings.preinit(this.dom);
			return shouldContinueAccordingToPreinit;
		}
		return true;
	},
	...
	reportError: function(anErrorString) {
		if (this.settings.error_sink)
			this.settings.error_sink.call(this, this.id(), anErrorString);
	},
