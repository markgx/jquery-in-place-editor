...
$.extend(InlineEditor.prototype, {
	...
	init: function() {
		...
		var that = this;
		this.dom
			.bind('mouseenter.editInPlace', function(){ $(this).css("background", that.settings.bg_over); })
			.bind('mouseleave.editInPlace', function(){ $(this).css("background", that.settings.bg_out); })
	...
	connectEventsToEditor: function() {
		...
		// BUG: this cancels even after the editor is closed (with commit)!! 
		//      Setting the event handler on the input or form should mitigate this problem
	...
	handleSubmitToServer: function(enteredText) {
		...
		$.ajax({
			...
			success: function(html){
				...
				var new_text = html || this.settings.default_text;
				
				/* put the newly updated info into the original element */
				this.dom.html(new_text);
				if (this.settings.success)
					this.settings.success(html, this.dom);
			},
			error: function(request) {
				this.dom.html(this.originalHTML); // REFACT: what about a restorePreEditingContent()
				if (this.settings.error)
					this.settings.error(request, this.dom);
			}
		});
	},
