	...
	createEditorElement: function() {
		...
		if(this.settings.field_type == "text") {
			...
			for(var i=0; i<optionsArray.length; i++){
				...
				var selected = (use_value == this.originalHTML) ? 'selected="selected" ' : '';
				var option = $('<option ' + selected + ' ></option>').val(use_value).text(optionsValuesArray[0]);
