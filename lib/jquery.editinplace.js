...
You select some elements with jQuery and then bind editInPlace to it like so

> $('#editable .table .cells').editInPlace(options)

where options can be all the options exposed as $(...).editInPlace.defaults. You can also change those to change options globally for your application.
...
$.extend(InlineEditor.prototype, {
	...
	connectEvents: function() {
		...
		this.dom
			...
			.bind('click.editInPlace', function(eventObject){ that.handleClickOnClosedEditor(eventObject); });
	},
	...
	handleClickOnClosedEditor: function(eventObject) {
		if ( ! this.shouldOpenEditor(eventObject.target))
			return;
	...
	shouldOpenEditor: function(eventTarget) {
		if (this.settings.cancel) {
			var isClickedObjectCancelled = this.isClickedObjectCancelled(eventTarget);
			if (true === isClickedObjectCancelled)
				return false;
		}
	...
	isClickedObjectCancelled: function(eventTarget) {
		var eventTargetAndParents = $(eventTarget).parents().add(eventTarget);
