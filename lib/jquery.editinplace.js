	...
	replaceContentWithEditor: function(settings) {
		...
		return function(){
			...
			function hookUpEvents() {
				that.dom.find("form")
					.find(".inplace_field").focus().select().end()
					.find(".inplace_cancel").click(cancelAction).end()
					.find(".inplace_save").click(saveAction);
				...
				if ( ! settings.show_buttons) {
					if ("save" === settings.on_blur)
						...
						that.dom.children("form").children(".inplace_field").blur(saveAction);
					else
						that.dom.children("form").children(".inplace_field").blur(cancelAction);
				}
				...
				that.dom.children("form").submit(saveAction);
			}

			function cancelAction() {
				editing = false;
				that.dom.css("background", settings.bg_out);
			...
			function saveAction() {
				...
				if (settings.callback) {
					html = settings.callback(that.dom.attr("id"), new_html, that.originalHTML, settings.params);
					editing = false;
					if (html)
						that.dom.html(html);
					else {
					...
					$.ajax({
						url: settings.url,
						type: "POST",
						data: settings.update_value + '=' + new_html + '&' + settings.element_id + '=' + that.dom.attr("id") + settings.params + '&' + settings.original_html + '=' + that.originalHTML,
						dataType: "html",
						...
						success: function(html){
							...
							if (settings.success) settings.success(html, that.dom);
						},
						error: function(request) {
							that.dom.html(that.originalHTML);
							if (settings.error) settings.error(request, that.dom);
						}
					});
				}
